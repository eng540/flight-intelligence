...
class OpenSkyClient:
        ...
        try:
            with httpx.Client(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    time.sleep(60)  # زيادة وقت الانتظار عند الحظر
                    return None
                else:
    ...
    # ==========================================
    # الدوال القديمة (تم الاحتفاظ بها لضمان التوافق)
    # ==========================================
    ...
    # ==========================================
    # الدوال الجديدة (تمت إضافتها لتلبية متطلبات الـ PDF)
    # ==========================================

    def get_flights_by_time(self, begin: int, end: int) -> List[Dict[str, Any]]:
        """(Alias) جلب جميع الرحلات في فترة زمنية محددة"""
        return self.get_all_flights(begin, end)
